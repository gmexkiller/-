...
import { CheckCircle2, Gauge, LoaderCircle, Play, Route, Send, Sparkles } from 'lucide-react';

import { RouteMap } from '@/components/classroom/route-map';
...
export function RoutePlanner({
  ...
  const [plan, setPlan] = useState<RoutePlan>(() => initial || DEFAULT_ROUTE_PLAN);
  const [animateKey, setAnimateKey] = useState(0);
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState('');
  const metrics = useMemo(() => calculateRouteMetrics(plan), [plan]);
  const valid = plan.evidenceTags.length > 0 && plan.reason.trim().length >= 4;
    ...
    <div className={compact ? 'grid min-h-0 grid-cols-[1.1fr_0.9fr] gap-5' : ''}>
      <div>
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          <p className="text-xs font-bold leading-5 text-slate-500">拖动 1–3 号节点左右移动；每个节点位于固定高度，路线会一直向上。</p>
        ...
        <div className="mt-3 grid grid-cols-3 gap-2">
          <Metric label="路线类型" value={metrics.routeType.replace('路线', '')} icon={<Route className="size-4" />} />
          <Metric label="相对路程" value={metrics.lengthLabel} detail={`${metrics.lengthRatio} 倍`} icon={<Sparkles className="size-4" />} />
          <Metric label="最陡路段" value={metrics.steepnessLabel} icon={<Gauge className="size-4" />} />
        </div>
      </div>
